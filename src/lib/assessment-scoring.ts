...
export function calculateAssessmentScores(answers: AssessmentAnswers): AssessmentScores {
  ...
  const metabolicBonusMultiplier =
    ageRange === '18-30' ? 1.04 : ageRange === '60+' ? 0.95 : 1;

  const recoveryPenaltyMultiplier =
    ageRange === '18-30' ? 0.95 : ageRange === '46-60' ? 1.08 : ageRange === '60+' ? 1.16 : 1;

  const recoveryBonusMultiplier =
    ageRange === '18-30' ? 1.05 : ageRange === '60+' ? 0.94 : 1;

  const pushInsight = (insight: AssessmentScores['insights'][number]) => {
  ...
  const insightPriority: Record<AssessmentScores['insights'][number]['flag'], number> = {
